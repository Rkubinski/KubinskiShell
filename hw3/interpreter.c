...
void exec(char*command[], int args)
	...
	char* alreadyLoaded[3];
	int alreadyLoadedFlag=0;
	int counter=0;
	...
	for (int i= 1; i<args;i++)
		{
			for (int j=0; j<counter;j++)
				{	
					...
					if (strcmp(command[i],alreadyLoaded[j])==0)
			...
			if (alreadyLoadedFlag!=1)
				{FILE *f = fopen(command[i],"r");
				myinit(f);
				}
		alreadyLoadedFlag=0;
		alreadyLoaded[i-1]=command[i];
		counter++;
		}
	...
	scheduler();
	//erasing the array that was filled with file names
